...
const SaveRestore = () => {
  ...
  eventAddNode.on('addNode', (randomId: string) => onAdd(randomId));
  const [nodeCount, setNodeCount] = useState(1);
  const onAdd = (randomId: string) => {
    const nodes = getNodes();
    // 如果nodes中id为randomId的节点存在，则不添加新节点
    if (nodes.some(node => node.id === randomId)) {
      return;
    }
    const newNode: PositionLoggerNode = {
      id: randomId ? randomId : getNodeId(),
