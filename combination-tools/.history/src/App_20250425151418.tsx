...
const SaveRestore = () => {
  ...
  */
  ...
  useEffect(() => {
    const handleAddNode = (randomId: string) => onAdd(randomId);
    eventAddNode.on('addNode', handleAddNode);
    return () => {
      eventAddNode.off('addNode', handleAddNode);
    }
  }, [])
  const onAdd = ({randomId: string,e}) => {
    ...
    const filteredEdges = getEdges().filter((edge) => edge.id !== 'a->b');
    const left = { id: `edgesL_${new Date()}`, type: 'custom-edge',source: 'a', target: randomId }
    const right = { id: `edgesR_${new Date()}`, type: 'custom-edge',source: randomId, target: 'b'}
  ...
  return (
    ...
    >
      ...
      <Panel className='top-right' position="top-right">
        ...
        <Button onClick={() => onAdd(getNodeId())}>添加节点</Button>
