...
const SaveRestore = () => {
  ...
  const onExecute = async () => {
    ...
    const res:{code:number,data:Object} = await axios.post(`${baseURL}/url`, EdgesAndNodes, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
    if (res.code === 200) { setNodes(res.data) } else { messageApi.open({ type: 'error', content: '执行失败' }); }
  }
