...
export function StartorEndNode({
    ...
    const getTree = (edges = getEdges(), nodes = getNodes()) => {
        ...
        const result = getBeforeNode(edges, id); // 通过边来获取到前面有哪些节点id
        // console.log(edges);

        // console.log('StartresultEnd', result, edges.length);
        
        const tree = getTreeList(result, nodes); // 通过节点id来获取到节点信息，并过滤拿到选择列表
        setTreeData(tree)
    }
            ...
            >
                ...
                {data.inputs?.map((item) => {
                    return <Flex style={{ backgroundColor: '#f5f5f5', padding: '4px', borderRadius: '4px' }} gap="small" wrap key={item.key} className='margin-top' align='center'>
                        ...
                        {item.isFold ?
                            <Avatar style={{ position: 'relative', left: '150px' }} onClick={() => updateIsFold(item.key)} className='avatar' src={Fold}></Avatar> :
                            <Avatar style={{ position: 'relative', left: '150px', padding: '2px' }} onClick={() => updateIsFold(item.key)} className='avatar' src={Unfold}></Avatar>
                        }
