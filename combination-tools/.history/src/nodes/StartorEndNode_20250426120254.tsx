            ...
            >
                ...
                {data.inputs?.map((item) => {
                    return <Flex style={{ backgroundColor: '#f5f5f5', padding: '4px', borderRadius: '4px' }} gap="small" wrap key={item.key} className='margin-top' align='center'>
                        ...
                        {item.isFold ?
                            <Avatar style={{ position: 'relative', left: '180px' }} onClick={() => updateIsFold(item.key)} className='avatar' src={Fold}></Avatar> :
                            <Avatar style={{ position: 'relative', left: '170px', padding: '2px' }} onClick={() => updateIsFold(item.key)} className='avatar' src={Unfold}></Avatar>
                        }
                        <Avatar style={{ position: 'relative', left: '190px' }} onClick={() => handleDelete(item.key)} className='avatar' src={deleteInput}></Avatar>
