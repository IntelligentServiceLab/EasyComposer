      ...
      >
        ...
        {data.outputs.map((item) => {
          return <Flex style={{ width: "100%", backgroundColor: '#f5f5f5', padding: '4px', borderRadius: '4px' }} wrap key={item.key} gap="small" className='margin-top' align='center'>
            ...
            {item.isFold ?
              <Avatar style={{ position: 'relative', left: '158px' }} onClick={() => updateIsFold(item.key, "输出")} className='avatar' src={Fold}></Avatar> :
              <Avatar style={{ position: 'relative', left: '150px', padding: '2px' }} onClick={() => updateIsFold(item.key, "输出")} className='avatar' src={Unfold}></Avatar>
            }
