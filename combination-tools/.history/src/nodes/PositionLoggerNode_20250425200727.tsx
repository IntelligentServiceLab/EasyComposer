...
export function PositionLoggerNode({
  ...
  useEffect(() => {
    ...
    // 在后面的设置中，如果遇到重复的参数，那就要加上参数出现的次数了
    data.inputs?.map((item: any) => {
      setTreeValues(prevState => {
        const newState = new Map(prevState);
        newState.set(`${item.key}_${item?.value?.name}`, String(`${item?.value.name}·${item?.value.input}·${item?.value.type}`));
        return newState;
      });
      // 直接在 map 之外获取更新后的值
      // console.log("Updated treeValues:", treeValues);
    });
  ...
  const onChangeTreeSelect = (selectedValue: string, treeKey: string,name: string) => {
    setTreeValues(prevState => {
      const newState = new Map(prevState);
      newState.set(`${treeKey}_${item?.value?.name}`, String(`${item?.value.name}·${item?.value.input}·${item?.value.type}`));
      return newState;
    });
    // 直接在 map 之外获取更新后的值
    // console.log("Updated treeValues:", treeValues);
        ...
        {data.inputs?.map((item) => {
          return <Flex style={{ width: "98%", backgroundColor: '#f5f5f5', padding: '4px', borderRadius: '4px' }} wrap key={item.key} gap="small" className='margin-top' align='center'>
            ...
            <TreeSelect
              ...
              onChange={(value) => onChangeTreeSelect(value, `${item.key}_${item?.value?.name}`,item?.value?.name)}
              treeData={treeData}
            />
