...
export function PositionLoggerNode({
  ...
  useEffect(() => {
    ...
    eventBusEdge.on('edgeUpdated', getTree);
  }, [])
  ...
  const onChangeTreeSelect = (selectedValue: string, treeKey: string, event: any) => {
    console.log(event);
    
    setTreeValues(prevState => new Map(prevState).set(treeKey, `${event?.value?.name}·${selectedValue}`)); // 这里加上名字
        ...
        {data.inputs?.map((item) => {
          return <Flex style={{ width: "98%", backgroundColor: '#f5f5f5', padding: '4px', borderRadius: '4px' }} wrap key={item.key} gap="small" className='margin-top' align='center'>
            ...
            <TreeSelect
              ...
              onChange={(value) => onChangeTreeSelect(value, item.key,item)}
              treeData={treeData}
            />
            {item.isFold ?
